<?php
class Constants
{
    public static $passwordsDoNotMatch = "Your passwords don't match";
    public static $passwordNotAlphanumeric = "Your password can only contain letters and numbers";
    public static $passwordLength = "Your password must be between 6 and 30 characters";
    public static $usernameLength = "Your username must be between 5 and 25 characters";
    public static $firstNameLength = "Your first name must be between 5 and 25 characters";
    public static $lastNameLength = "Your last name must be between 5 and 25 characters";
    public static $emailsDoNotMatch = "Your emails don't match";
    public static $invaildEmails = "Invalid Email";
    public static $emailTaken = "This email has already been used";
    public static $usernameTaken = "This username already exists";

    public static $loginFailed = "Login attempt failed";
}
